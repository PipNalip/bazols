...
### Reconciliation для issue #37 — 2026-09-17

Повторная ограниченная read-only проверка на разрешённой учётной записи подтвердила без сохранения идентификаторов, названий и payload:

- live product AutoCost возвращает непустые product-scoped строки и положительный `autoCost` по unit;
- live material AutoCost использует единственный код валюты `1`;
- загруженный клиент источника определяет `Ruble = 1` и форматирует его как ISO `RUB`/символ `₽`;
- продажи проверенного restaurant/unit уже имеют валюту `RUB`.

Для Level 1 отчёта #37 официальный product `autoCost` считается стоимостью одной проданной позиции продукта в валюте активного restaurant/unit. Расчёт разрешён только для продаж `RUB`; любая другая валюта должна приводить к безопасному отказу, а не к неявной конвертации. Это подтверждает COGS/gross-margin путь, но не подтверждает `reportedPrice` как текущую цену меню и не разблокирует #38.
...
3. #37 разблокирован после #36 и reconciliation 2026-09-17; расчёт ограничен подтверждённой валютой `RUB`.
