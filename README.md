...
Сейчас реализованы локальный health-срез и внутренний pipeline интеграции с источником: GET-only клиент с cookie-сессией, строгая проверка ответов, пагинация, шифрование raw snapshots и идемпотентная нормализация в PostgreSQL. Pipeline пока не запускается через API: очередь и worker добавляются следующим этапом. Production ещё не развёрнут; оставшиеся функции и инфраструктурные задачи перечислены в `docs/backlog.md` и спецификациях `docs/specs/`.
